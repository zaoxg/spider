...
def write_csv(i):
    ...
    with open('data/' + '1688' + '.csv', 'a+', encoding='utf-8', newline='') as csvfile:
        writer = csv.writer(csvfile)
        # 以读的方式打开csv 用csv.reader方式判断是否存在标题
        with open('data/' + '1688' + '.csv', 'r', encoding='utf-8', newline='') as f:
            reader = csv.reader(f)
            if not [row for row in reader]:
                writer.writerow(['商品id', '标题', '价格', '计量单位', '销量', '封面图', '生产商', '商品链接', '商家店铺链接'])
                writer.writerow(i)
            else:
                writer.writerow(i)
